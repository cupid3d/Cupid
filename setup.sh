# Read Arguments
TEMP=`getopt -o h --long help,new-env,basic,train,xformers,flash-attn,diffoctreerast,vox2seq,spconv,mipgaussian,kaolin,nvdiffrast,demo,pytorch3d,moge -n 'setup.sh' -- "$@"`
...
if [ "$HELP" = true ] ; then
    ...
    echo "  --pytorch3d             Install PyTorch3D (requires CUDA build)"
    echo "  --moge                  Install MoGe, Blender Python, and blender toolbox"
    return
fi

if [ "$NEW_ENV" = true ] ; then
    conda create -n cupid python=3.11
...
case $PLATFORM in
    ...
    hip)
        ...
        echo "[SYSTEM] Installing PyTorch 2.4.1 for HIP ($PYTORCH_VERSION -> 2.4.1+rocm6.1)"
            pip install torch==2.4.1 torchvision==0.19.1 --index-url https://download.pytorch.org/whl/rocm6.1 --user --no-cache-dir --no-build-isolation 
    ...
    if [ "$PLATFORM" = "cuda" ] ; then
        if [ "$CUDA_VERSION" = "11.8" ] ; then
            case $PYTORCH_VERSION in
                ...
                2.4.0) pip install xformers==0.0.27.post2 --index-url https://download.pytorch.org/whl/cu124 ;;
...
if [ "$KAOLIN" = true ] ; then
    ...
    if [ "$PLATFORM" = "cuda" ] ; then
        case $PYTORCH_VERSION in
            ...
            2.4.0) pip install kaolin -f https://nvidia-kaolin.s3.us-east-2.amazonaws.com/torch-2.4.0_cu124.html;;
...
if [ "$NVDIFFRAST" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        ...
        pip install /tmp/extensions/nvdiffrast --no-cache-dir --no-build-isolation 
    else
...
if [ "$DIFFOCTREERAST" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        mkdir -p /tmp/extensions
        git clone --recurse-submodules https://github.com/hbb1/diffoctreerast.git /tmp/extensions/diffoctreerast
        pip install /tmp/extensions/diffoctreerast --no-cache-dir --no-build-isolation 
    else
...
if [ "$MIPGAUSSIAN" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        ...
        pip install /tmp/extensions/mip-splatting/submodules/diff-gaussian-rasterization/ --no-cache-dir --no-build-isolation 
    else
...
if [ "$VOX2SEQ" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        ...
        pip install /tmp/extensions/vox2seq --no-cache-dir --no-build-isolation 
    else
...
if [ "$PYTORCH3D" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        VERSION_STR=$(python - <<'PY'
import sys
import torch
if torch.version.cuda is None:
    raise SystemExit("CUDA build of PyTorch is required for PyTorch3D wheels")
...
PY
        ...
        if [ -z "$VERSION_STR" ] ; then
            echo "[PYTORCH3D] Failed to determine compatible wheel"
        else
            pip install iopath
            pip install --no-index --no-cache-dir pytorch3d -f "https://dl.fbaipublicfiles.com/pytorch3d/packaging/wheels/${VERSION_STR}/download.html"
        fi
    else
        echo "[PYTORCH3D] Unsupported platform: $PLATFORM"
    fi
fi
