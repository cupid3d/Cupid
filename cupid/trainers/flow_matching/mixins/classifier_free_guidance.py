import torch
import numpy as np
from ....utils.general_utils import dicts_foreach
    ...
    def get_cond(self, cond, neg_cond=None, **kwargs):
        ...
        if self.p_uncond > 0:
            ...
            def select(cond, neg_cond, mask):
                if isinstance(cond, torch.Tensor):
                    mask = torch.tensor(np.array(mask), device=cond.device).reshape(-1, *[1] * (cond.ndim - 1))
            ...
            if not isinstance(cond, dict):
                ...
                cond = dicts_foreach([cond, neg_cond], lambda x: select(x[0], x[1], mask))
